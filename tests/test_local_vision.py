...
def test_local_parser_understands_character_count_ranges() -> None:
    document = OcrDocument(
        width=2048,
        height=960,
        tokens=[
            token("找出并处决2-4名恶徒", 80, 80),
            token("(2-4个爪牙和0-1个恶魔)", 90, 125),
            token("杀死恶徒：1/2-4", 80, 165),
            *[token(f"#{position}", position * 180, 300) for position in range(1, 10)],
            token("10", 100, 800),
        ],
    )
    ...
    assert result.config.card_count == 9
    assert result.config.evil_count == 2
    assert result.config.evil_count_max == 4
    assert result.config.minion_count == 2
    assert result.config.minion_count_max == 4
    assert result.config.demon_count == 0
    assert result.config.demon_count_max == 1


def test_local_parser_infers_card_count_when_one_seat_label_is_missed() -> None:
    document = OcrDocument(
        width=1800,
        height=900,
        tokens=[
            token("找出并处决2-4名恶徒", 20, 20),
            token("2-4个爪牙和0-1个恶魔", 20, 60),
            *[
                token(f"#{position}", position * 150, 250)
                for position in [1, 2, 3, 4, 5, 6, 8, 9]
            ],
            token("10", 60, 760),
        ],
    )
...
def test_local_parser_uses_card_geometry_for_real_round_table_layout() -> None:
    ...
    document = OcrDocument(
        ...
        height=1440,
        tokens=[
            centered_token("#8", 1281, 82),
            centered_token("#7", 947, 219),
            centered_token("#1", 1612, 220),
            centered_token("有1对恶徒", 746, 290),
            centered_token("建筑师", 1280, 293),
            centered_token("左边有更多恶", 1810, 290),
            centered_token("两两相邻", 745, 325),
            centered_token("徒", 1812, 325),
            centered_token("左边有更多恶", 1281, 377),
            centered_token("徒", 1281, 412),
            centered_token("针织者", 948, 430),
            centered_token("建筑师", 1612, 430),
            centered_token("#6", 810, 552),
            centered_token("#2", 1749, 552),
            centered_token("#1、#3、#8", 608, 587),
            centered_token("之中有：", 601, 622),
            centered_token("爪牙、村民和", 608, 658),
            centered_token("边缘人", 608, 693),
            centered_token("主教", 810, 763),
            centered_token("#5", 948, 884),
            centered_token("#3", 1612, 884),
            centered_token("#6是真的", 1281, 922),
            centered_token("主教", 1282, 958),
            centered_token("#4", 1281, 1022),
            centered_token("倒霉鬼", 947, 1095),
            centered_token("骑士", 1612, 1095),
